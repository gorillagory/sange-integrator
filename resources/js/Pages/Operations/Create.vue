        ...
        <template #breadcrumbs>
            <Breadcrumbs
                :items="[
                    ...
                    { label: 'Create Operation', url: null },
                ]"
            />
        </template>

        <div class="mb-8">
            <h1 class="text-2xl font-bold text-gray-900">Create Operation</h1>
            <p class="mt-1 text-sm text-gray-500">
                Capture routing, dynamic service payloads, and commercial figures in one operational flow.
            </p>
        </div>
            ...
            <div class="space-y-6 xl:col-span-8">
                ...
                >
                    <div class="mb-6 flex items-center gap-2">
                        ...
                        <h3 class="font-bold text-gray-900">Service Payloads & Commercial Values</h3>
                    </div>

                    <div class="mb-6">
                        <label class="mb-2 block text-xs font-bold uppercase text-gray-600">
                            Select Service Vector
                        </label>
                        ...
                        <div class="flex gap-2">
                            ...
                            >
                                ...
                                <option
                                    ...
                                    :value="schema.id"
                                >
                                    {{ schema.service_name || schema.display_name }}
                                </option>
                            </select>
                    ...
                    >
                        <ServiceRow
                            ...
                            :key="item.__uuid"
                            :item="item"
                            :schema="findSchemaForService(item)"
                    ...
                    >
                        Add at least one service vector to begin building the operation payload.
                    </div>
                </div>
            </div>
...
const form = useForm({
    handler_key: props.handler?.handler_key || null,
    client_id: null,
    contract_no: null,
    services: [],
});
...
function addService() {
    ...
    form.services.push({
        __uuid: `${schema.service_code}_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
...
function findSchemaForService(service) {
    if (!service) {
        return null;
    }

    if (service.service_schema_id) {
        const foundById = props.schemas.find((schema) => schema.id === service.service_schema_id);
        if (foundById) {
            return foundById;
        }
    }
    ...
    if (service.service_code) {
        const foundByCode = props.schemas.find((schema) => schema.service_code === service.service_code);
...
function submitOperation() {
    form.transform((data) => ({
        ...
        services: data.services.map((service) => ({
            service_schema_id: service.service_schema_id ?? null,
            service_code: service.service_code,
            service_details: sanitizeServiceDetails(service.service_details),
            service_details_extra: service.service_details_extra || {},
